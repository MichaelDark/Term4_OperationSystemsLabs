...
void CryptMessage(wchar_t * msg, int length, ulong e, ulong n, ulong * crypto)
{
...
void DecryptMessage(wchar_t * msg, int length, ulong d, ulong n, ulong * crypto)
{
	for (int i = 0; i < length; i++)
	{
		msg[i] = static_cast<wchar_t>(ModularPow(crypto[i], d, n));
	}
